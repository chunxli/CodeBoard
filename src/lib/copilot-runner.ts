...
function buildArgs(opts: StartCopilotRunOptions, model: string | null): string[] {
...
export async function startCopilotRun(opts: StartCopilotRunOptions): Promise<{
  ...
  return new Promise((resolve) => {
    const startAttempt = (model: string | null) => {
      ...
      child.on("close", (code) => {
        ...
        if (shouldFallback) {
          ...
          void appendFile(logPath, `${notice}\n`).catch(() => {});
          startAttempt(fallbackModel);
          return;
        }
